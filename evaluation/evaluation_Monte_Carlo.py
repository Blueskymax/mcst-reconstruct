...
def evaluation(model, args):
    ...
    tarTracks.track_init(detections, args.predictor_time_series_len, args.predictor_in_features, args.eval_batch_size, args.T, args.device)
    ...
    for frame_index in range(args.predictor_time_series_len, detections.shape[1]):  #开始当前sequence
        ...
        # 更新 predict update
        predict_output_data = minMaxScaler.de_min_max_scaler(
            output_normalized_predict.unsqueeze(dim=2)).squeeze(
            dim=2)
        update_output_data = minMaxScaler.de_min_max_scaler(output_normalized_update.unsqueeze(dim=2)).squeeze(
            dim=2)
