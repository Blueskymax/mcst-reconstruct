...
class MyMinMaxScaler(nn.Module):
    ...
    def forward(self, *args, mode: str):
        # 最小最大归一化
        # labels (batch, len, dim) input_detection (batch, len, dim)
        # 沿着时间维度的方向计算最小值和最大值
        self.mode = mode
        max_velocity = args[-1]
        if len(args) == 5:
            ...
            scaled_estimation = (estimation - min_vals) / denominator
            ...
            if dim_size == 3:
                ...
                raise Exception("MyMinMaxScaler error!")
    ...
    def de_min_max_scaler(self, x):
        if self.mode == "0_1":
            x_output = ((x * 1) + 0) * (self.max_vals - self.min_vals) + self.min_vals
            return x_output
        elif self.mode == "-1_1":
            x_output = ((x * 0.5) + 0.5) * (self.max_vals - self.min_vals) + self.min_vals
            return x_output
        else:
            raise Exception("MyMinMaxScaler error!")